...
export class MainPageComponent implements OnInit {
    ...
    this.ApiService.getData().subscribe(
      (res: any) => {
        for (const property in res) {
          if (property != "State Unassigned") {
            this.stateList.push(property);
          }
        }
        this.API_DATA = res;
      },
  ...
  handleStateClick(state:any): void {
    ...
    distNameList.forEach((itm: any, i) => {
      ...
      this.finalStateData["stateStatus"].stateConfirmed +=
        tempDistData?.confirmed;
      this.finalStateData["stateStatus"].stateDeceased +=
        tempDistData?.deceased;
      this.finalStateData["stateStatus"].statRecovered +=
        tempDistData?.recovered;
